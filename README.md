...
- **Actividad física por día:** switch "¿Entrenaste hoy?" y, si está activo, campos para tipo de actividad, duración, **tipo de recuperación** y **si notaste mejorías en el rendimiento** — los puntos exactos que pide la consigna.
- **Descanso por día** (independiente de si entrenaste): **calidad del descanso** (escala 1 a 5) y notas adicionales, siempre disponible.
- **Agua:** botón "💧 + Agregar agua" que registra la hora actual (no editable) y una opción a elegir (Vaso chico / Vaso grande / Botella 500ml). Cada registro se intercala automáticamente entre las comidas según su horario (por ejemplo, un vaso a las 6 aparece antes del desayuno de las 7; uno a las 8, entre desayuno y media mañana).
- **Enviar por mail a tu nutricionista:** genera un PDF (nombrado con tu nombre y la fecha de generación) con los 3 días (comidas con horario y foto embebida, más el registro de entrenamiento) y abre la app de mail del celular con el correo prearmado, el PDF adjunto, y asunto/cuerpo ya completados — solo falta tocar "Enviar". Si el celular no tiene una app de mail configurada, cae automáticamente al menú de compartir nativo (WhatsApp, etc.).
- **Historial (tab 2):** lista de períodos de registro anteriores, para reutilizar la app en cada control futuro.
- **Tus datos** (link "⚙ Tus datos" en la pantalla de Registro): tu nombre (se usa para el PDF; si no está completo, la app te lo pide antes de exportar) y el nombre/mail de tu nutricionista, editables — vienen precargados con los de Pilar Olaverry pero se pueden cambiar.
